...
{   
    ...
    for (int k = 0; k < (int)wp.poses.size(); k++)
    {
        ...
        if(wp.poses[k].pose.position.z < 0.0)// the end condition when waypoint's z value small than 0
            break;
    }
    //MatrixXd's first waypoint is the initial state ,the others are the waypoint state,
...
int main(int argc, char** argv)
{
    ...
    _poly_num1D = 2 * _dev_order;//get from the launch file, the value is 4
    ...
    _wp_traj_vis_pub = nh.advertise<visualization_msgs::Marker>("vis_trajectory", 1);//publish the trajectory
    _wp_path_vis_pub = nh.advertise<visualization_msgs::Marker>("vis_waypoint_path", 1);//publish the waypoint and line
...
{        
    ...
    {   
        for (double t = 0.0; t < time(i); t += 0.01, count += 1)
        {
          ...
          if (count) traj_len += (pre - cur).norm();//.norm is calculate the distance between 2 points
          pre = cur;
        }
    }
...
Vector3d getPosPoly( MatrixXd polyCoeff, int k, double t )//k is the row of the time
{
    Vector3d ret;

    for ( int dim = 0; dim < 3; dim++ )
    {   // segment 获取从向量的第i个元素开始的n个元素 这里分别获取xyz轴的系数
        //k 代表第几行
        VectorXd coeff = (polyCoeff.row(k)).segment( dim * _poly_num1D, _poly_num1D );//_poly_num1D the order of the polynominal
        ...
        ret(dim) = coeff.dot(time);//dot向量点乘
...
VectorXd timeAllocation( MatrixXd Path)//path is the waypoint,the format of the path is the number of the waypoint +1
{ 
    ...
    */
   for(int i=0;i<time.rows();i++){//this need the segment of time rather than the number of the waypoint
       
       double dis=sqrt(pow((Path(i,0)-Path(i+1,0)),2)+  //x
                       pow((Path(i,1)-Path(i+1,1)),2)+  //y 
                       pow((Path(i,2)-Path(i+1,2)),2)); //z

       double time_acc=_Vel/_Acc;
       double dis_acc=0.5*_Acc*time_acc*time_acc;
       //becasue the deaccelerate process is same with the accelerate process
       
       time(i)=  ((dis-2*dis_acc)/_Vel)+2*time_acc; //the time is the sum of uniform motion time and the accelerate and deaccelerate time
   }
