...
n_seg         = size(path,1)-1;% segment number n point just have n-1 segements
n_poly_perseg = (n_order+1); % coef number of perseg

ts = zeros(n_seg, 1);% n row 1 column
...
%     Pxi=fliplr( poly_coef_x(i*8+1:i*8+8));
%     Pyi=fliplr( poly_coef_y(i*8+1:i*8+8));
    Pxi=poly_coef_x(i*8+1:i*8+8);
    Pyi=poly_coef_y(i*8+1:i*8+8);
