...
T.v(1).indPrev = 0;     %节点数存的是离他最近的节点索引，方便后面回溯
%% 开始构建树——作业部分
figure(1);
...
for iter = 1:3000
    ...
    count=count+1;%节点计数这里节点数量就是有效节点即为T中节点数量
    %Step 4: 将x_new插入树T 
    %提示：新节点x_new的父节点是x_near
    T.v(count).x = x_new(1,1);         % T是我们要做的树，v是节点，这里先把起始点加入到T里面来
    T.v(count).y = x_new(1,2); 
    T.v(count).xPrev = x_near(1,1);     % 起始节点的父节点仍然是其本身
