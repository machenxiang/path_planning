...
public:
    ...
    {   
        ...
        // example, so we downcast state into the specific type.降状态到指定类型
    ...
    */ 
    ...
    if (solved)
    {
        ...
        for (size_t path_idx = 0; path_idx < path->getStateCount (); path_idx++)
        {
            ...
            */ 
           Vector3d path_pt;
           path_pt(0)=state->values[0];
           path_pt(1)=state->values[1];
           path_pt(2)=state->values[2];
...
int main(int argc, char** argv)
{
    ...
    _grid_map_vis_pub             = nh.advertise<sensor_msgs::PointCloud2>("grid_map_vis", 1);//获取目标点
    _RRTstar_path_vis_pub         = nh.advertise<visualization_msgs::Marker>("RRTstar_path_vis",1);//获取地图
